var index =
[
    [ "Table of Contents", "index.html#table-of-contents-3", null ],
    [ "<b><em>New</em></b> Complete C-Menu HTML Documentation", "index.html#_new_-complete-c-menu-html-documentation", null ],
    [ "C-Menu Exercises", "index.html#c-menu-exercises", null ],
    [ "Menu, Form, Pick, View, C-Keys, RSH, and lf", "index.html#menu-form-pick-view-c-keys-rsh-and-lf", [
      [ "Elements of C-Menu", "index.html#elements-of-c-menu", null ],
      [ "Key Features", "index.html#key-features", null ],
      [ "Highlights (Pun Intended)", "index.html#highlights-pun-intended", null ],
      [ "Just Do It!", "index.html#just-do-it", null ],
      [ "Menu", "index.html#menu-1", null ],
      [ "C-Keys - Diagnose Keyboard/Mouse Input", "index.html#c-keys---diagnose-keyboardmouse-input", null ],
      [ "Pick - A Picker", "index.html#pick---a-picker", null ],
      [ "Form for Data Entry and Editing", "index.html#form-for-data-entry-and-editing", null ],
      [ "Form Key Features", "index.html#form-key-features", null ],
      [ "Form Data Types", "index.html#form-data-types-1", null ],
      [ "Sample Menu Description File", "index.html#sample-menu-description-file", null ],
      [ "View", "index.html#view", [
        [ "Log File With Editor", "index.html#log-file-with-editor", null ],
        [ "Screenshot of the Same File With View", "index.html#screenshot-of-the-same-file-with-view", null ]
      ] ],
      [ "View and Color Manual Pages", "index.html#view-and-color-manual-pages", null ],
      [ "Example Manual Page Using Editor", "index.html#example-manual-page-using-editor", null ],
      [ "Example Manual Page", "index.html#example-manual-page", null ],
      [ "Demand Paged Virtual Address Space", "index.html#demand-paged-virtual-address-space", null ],
      [ "View - Other Features", "index.html#view---other-features", null ],
      [ "3-Channel Gamma Correction", "index.html#autotoc_md3-channel-gamma-correction", null ],
      [ "Gray Gamma", "index.html#gray-gamma", null ],
      [ "RSH", "index.html#rsh", null ],
      [ "Maintaining Security with RSH", "index.html#maintaining-security-with-rsh", null ],
      [ "lf - List Files Using Regular Expressions", "index.html#lf---list-files-using-regular-expressions", null ]
    ] ],
    [ "Features", "index.html#features", null ],
    [ "Configuration", "index.html#configuration", null ],
    [ "MIT License", "index.html#mit-license-1", null ]
];